...
void RunTask() {
  ...
  const int K = 2;
  const int O = (X - K) / 2 + 1;
  mshadow::TensorContainer<mshadow::cpu, 4, float> srcm(
      mshadow::Shape4(1, 1, X, X));
  Xmshadow::TensorContainer<Xmshadow::cpu, 4> srcx(
      Xmshadow::Shape4(1, 1, X, X));
  for (int i = 0; i < X; ++i) {
    for (int j = 0; j < X; ++j) {
  ...
  printf("\n");
  mshadow::TensorContainer<mshadow::cpu, 4, float> gpool_src(
      mshadow::Shape4(1, 1, O, O));
  Xmshadow::TensorContainer<Xmshadow::cpu, 4> gpool_xsrc(
      Xmshadow::Shape4(1, 1, O, O));
  for (int i = 0; i < O; ++i) {
    for (int j = 0; j < O; ++j) {
  ...
  mshadow::TensorContainer<xpua, 4, float> gpool_ct(
      mshadow::Shape4(1, 1, O, O));
  ...
  mout = mshadow::expr::unpool<mshadow::red::maximum>(mct, pool_ct, gpool_ct, K,
                                                      K, K);
  xout = Xmshadow::expr::unpool<Xmshadow::red::maximum>(xct, pool_xct,
                                                        gpool_xct, K, K);
