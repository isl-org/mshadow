...
void RunTask() {
  ...
  const int kstride = 2;
  const int X = 6;
  Xmshadow::TensorContainer<Xmshadow::cpu, 4> xsrc(
  ...
  xtmp_col.Resize(Xmshadow::Shape2(xin.shape[2] * ksize * ksize,
                                   nbatch * oheight * owidth));
  tmp_col.Resize(
      mshadow::Shape2(in.size(1) * ksize * ksize, nbatch * oheight * owidth));
  ...
  xtc.Resize(Xmshadow::Shape2(xin.shape[2] * ksize * ksize,
                              nbatch * oheight * owidth));
  tc.Resize(
      mshadow::Shape2(in.size(1) * ksize * ksize, nbatch * oheight * owidth));
