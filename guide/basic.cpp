...
int main(void) {
  ...
  // Tensor object is only a handle, assignment means they have same data
  // content we can specify content type of a Tensor, if not specified, it is
  // float bydefault
  ...
  TensorContainer<cpu, 2> lhs(Shape2(2, 3)), rhs(Shape2(2, 3)),
      ret(Shape2(2, 2));
  lhs = 1.0;
  rhs = 1.0;
