...
struct relu_grad {
  MSHADOW_XINLINE static real_t Map(real_t a) { return a > 0.0f ? 1.0f : 0.0f; }
};
...
class INNet {
 public:
  virtual void Forward(const Tensor<cpu, 4, real_t> &inbatch,
...
class ConvNet : public INNet {
 public:
  ...
  ConvNet(int batch_size, int insize, int nchannel, int ksize, int kstride,
          int psize, int num_out)
    ...
    nhidden.Resize(Shape4(batch_size, nchannel, (insize - ksize) / kstride + 1,
                          (insize - ksize) / kstride + 1));
    nhiddenbak.Resize(nhidden.shape_);
    npool.Resize(Shape4(batch_size, nchannel,
                        (nhidden.size(2) + 1 - psize) / psize,
                        (nhidden.size(3) + 1 - psize) / psize));
    npoolbak.Resize(npool.shape_);
    nflat.Resize(
        Shape2(batch_size, npool.size(1) * npool.size(2) * npool.size(3)));
  ...
  virtual void Forward(const Tensor<cpu, 4, real_t> &inbatch,
    ...
    // max pooling
    npool =
        pool<red::maximum>(nhiddenbak, npool[0][0].shape_, psize, psize, psize);
  ...
  virtual void Backprop(const Tensor<cpu, 2, real_t> &gradout) {
    ...
    nhiddenbak =
        unpool<red::maximum>(nhiddenbak, npoolbak, npool, psize, psize, psize);
    ...
    ConvBackWard(nhidden, Ki2h, g_Ki2h, ninput, ksize, kstride, tmp_col,
                 tmp_dst);
  }
  // update weight
 ...
 private:
  ...
  inline static void ConvForward(const Tensor<xpu, 4, real_t> &in,
                                 ...
                                 Tensor<xpu, 4, real_t> &out, int ksize,
                                 int kstride,
    ...
    // this cost lots of memory, normally for large image, only unpack several
    // image at a time
    tmp_col.Resize(
        Shape2(in.size(1) * ksize * ksize, nbatch * oheight * owidth));
    ...
    out = swapaxis<1, 0>(
        reshape(tmp_dst, Shape4(nchannel, nbatch, oheight, owidth)));
  }
  ...
  inline static void ConvBackWard(const Tensor<xpu, 4, real_t> &out,
                                  ...
                                  Tensor<xpu, 4, real_t> &in, int ksize,
                                  int kstride,
                                  TensorContainer<xpu, 2, real_t> &tmp_col,
                                  TensorContainer<xpu, 2, real_t> &tmp_dst) {
    index_t oheight = (in.size(2) - ksize) / kstride + 1;
    ...
    in = pack_col2patch(tmp_col, in.shape_, ksize, ksize, kstride, kstride, 1,
                        1);
  }
 ...
 private:
  ...
  // weight, gradient: Ki2h is actually convoltuion kernel, with
  // shape=(num_channel,ksize*ksize)
...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(argv[1], "gpu")) {
    ...
    net = new ConvNet<gpu>(batch_size, insize, nchannel, ksize, kstride, psize,
                           num_out);
  } else {
    InitTensorEngine<cpu>();
    net = new ConvNet<cpu>(batch_size, insize, nchannel, ksize, kstride, psize,
                           num_out);
  }
  ...
  LoadMNIST("train-images-idx3-ubyte", "train-labels-idx1-ubyte", ytrain,
            xtrain_, true);
  LoadMNIST("t10k-images-idx3-ubyte", "t10k-labels-idx1-ubyte", ytest, xtest_,
            false);

  TensorContainer<cpu, 4, real_t> xtrain(
      Shape4(xtrain_.size(0), 1, insize, insize));
  TensorContainer<cpu, 4, real_t> xtest(
      Shape4(xtest_.size(0), 1, insize, insize));
