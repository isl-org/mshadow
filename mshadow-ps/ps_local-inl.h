...
class LocalModel : public ISharedModel<xpu, DType> {
 public:
  ...
  // destructor
  virtual ~LocalModel(void) { this->Destroy(); }
  ...
  virtual void SetParam(const char *name, const char *val) {
    ...
    if (!strcmp(name, "pull_thread")) {
      if (!strcmp(val, "ndev")) {
        ...
        LOG(FATAL) << "invalid value for parameter pull_thread,"
    ...
    if (!strcmp(name, "push_thread")) {
      if (!strcmp(val, "ndev")) {
        ...
        LOG(FATAL) << "invalid value for parameter push_thread,"
                   << " can only be ndev or one";
      }
    }
  ...
  virtual void PullWait(int key, int devid) {
    ...
    CHECK_EQ(e.wait.size(), devices.size())
  ...
  // set weight
  virtual void SetWeight_(Tensor<xpu, 2, DType> data, int key, int devid) {
  ...
  virtual void CheckWeight_(Tensor<xpu, 2, DType> data, int key, int devid) {
    ...
    if (rerr > 1e-5 || diff != diff) {
      fprintf(stderr,
              "PSLocal:key=%d,dev=%d: err=%f, maxd[%u]=%f, diff=%f, ssum=%f\n",
 ...
 protected:
  ...
  virtual void InitKey_(Shape<2> shape, int key, int devid) {
  ...
  virtual void PullReq_(Tensor<xpu, 2, DType> data, int key, int devid,
                        ...
                        void *callback_arg) {
    PullEntry &e = pull_map.GetRef(key);
    CHECK_EQ(e.req.size(), devices.size())
  ...
  virtual void HandlePushFinish(Tensor<cpu, 3, DType> data, int key) {
    ...
    LocalOp op = kSum;
    typename std::map<int, LocalOp>::const_iterator it =
        push_operation.find(key);
   ...
   * \param key the key of the data
   */
  inline void HandleReduceFinish(Tensor<cpu, 2, DType> data, int key) {
    if (custom_server != NULL) {
...
#if defined(_OPENMP)
    if (data[0].MSize() >= bigarray_bound && nthread_reduction != 0) {
...
#pragma omp parallel for schedule(static) num_threads(nthread_reduction)
 ...
 private:
  ...
  struct PushEntry {
    ...
    bool pin_memory;
    // constructor
    PushEntry(void) : copyin_version(0) { weight.dptr_ = NULL; }
    ~PushEntry(void) {
      if (data.dptr_ != NULL) {
        if (pin_memory) {
    ...
    inline void Init(int ndevice, Shape<2> shape, bool pin_memory,
      ...
      CHECK(!need_weight || weight.CheckContiguous())
    ...
    while (!destroy_signal) {
      ...
      if (queue->Pop(&tsk)) {
        ...
        CHECK_EQ(e.data[0][0].shape_, tsk.data.shape_)
            ...
            << e.data[0][0].shape_ << " vs " << tsk.data.shape_;
  ...
  inline void PushHandlerLocal(size_t tid) {
    ...
    CHECK_EQ(push_queues.size(), devices.size())
  ...
  inline void PullHandlerLocal(size_t tid) {
    CHECK_LT(tid, devices.size()) << "threadid exceed boundary";
    CHECK_EQ(pull_queues.size(), devices.size())
        << "must have one pull_queue per device";
    // allocate stream resources
    SetDevice<xpu>(devices[tid]);
  ...
  inline int GetWorkIndex(int devid) const {
    CHECK(devid >= 0 && devid < static_cast<int>(dev2index.size()) &&
          dev2index[devid] >= 0)
        << "Push: invalid devid";
