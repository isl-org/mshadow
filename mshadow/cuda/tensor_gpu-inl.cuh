...
#define MSHADOW_CUDA_POST_KERNEL_CHECK(x)                           \
  ...
  do {                                                              \
    ...
    CHECK_EQ(err, cudaSuccess)                                      \
        << "Name: " << #x << " ErrStr:" << cudaGetErrorString(err); \
  } while (0)
namespace mshadow {
namespace cuda {
...
inline void CheckLaunchParam(dim3 dimGrid, dim3 dimBlock,
                             const char *estr = "") {
  if (dimBlock.x * dimBlock.y * dimBlock.z >
          static_cast<unsigned>(kMaxThreadsPerBlock) ||
      dimGrid.x > kMaxGridDim || dimGrid.y > kMaxGridDim) {
    LOG(FATAL) << "too large launch parameter: " << estr << "[" << dimGrid.x
               << "," << dimGrid.y << "], [" << dimBlock.x << "," << dimBlock.y
               << "," << dimBlock.z << "]";
  }
}
template <typename Saver, typename DstPlan, typename Plan, int block_dim_bits>
__device__ void MapPlanProc(DstPlan dst, index_t xstride, Shape<2> dshape,
                            const Plan plan, int block_idx) {
...
template <typename Saver, int block_dim_bits, typename DstPlan, typename Plan>
__global__ void MapPlanKernel(DstPlan dst, index_t xstride, Shape<2> dshape,
                              const Plan plan) {
  MapPlanProc<Saver, DstPlan, Plan, block_dim_bits>(dst, xstride, dshape, plan,
                                                    blockIdx.x);
}
template <typename Saver, int block_dim_bits, int grid_size, typename DstPlan,
          typename Plan>
...
inline void MapPlan(expr::Plan<DstExp, DType> dst,
                    const expr::Plan<E, DType> &plan, Shape<2> dshape,
                    cudaStream_t stream) {
  const index_t xstride = GetAlignStride(dshape[1]);
  const int num_block =
      (dshape[0] * xstride + kBaseThreadNum - 1) / kBaseThreadNum;
  dim3 dimBlock(kBaseThreadNum, 1, 1);

  if (num_block < kMaxGridNum) {
    dim3 dimGrid(num_block, 1, 1);
    MapPlanKernel<Saver, kBaseThreadBits, expr::Plan<DstExp, DType>,
                  expr::Plan<E, DType>>
    ...
    MapPlanLargeKernel<Saver, kBaseThreadBits, kBaseGridNum,
                       expr::Plan<DstExp, DType>, expr::Plan<E, DType>>
        <<<dimGrid, dimBlock, 0, stream>>>(dst, xstride, dshape, plan, repeat);
    MSHADOW_CUDA_POST_KERNEL_CHECK(MapPlanLargeKernel);
  }
}

template <typename Saver, typename Reducer, int warp_bits, typename DType,
          typename DstPlan, typename Plan>
__global__ void __launch_bounds__(kMemUnit *kMemUnit, 1)
    MapRedKeepLowestKernel(DstPlan dst, Plan plan, DType scale,
                           Shape<2> eshape) {
  ...
  // note: reverse store [y][x], so that we can reduce over threadIdx.x, use
  // warp optimization
  ...
  for (unsigned y = warp_size; y < eshape[0]; y += warp_size) {
    if (threadIdx.y + y < eshape[0] && x < eshape[1]) {
      Reducer::Reduce(s_res[threadIdx.x][threadIdx.y],
                      plan.Eval(threadIdx.y + y, x));
    }
  }
  __syncthreads();
...
template <typename Saver, typename Reducer, typename DstExp, typename E,
          typename DType>
inline void MapReduceKeepLowest(expr::Plan<DstExp, DType> dst,
                                const expr::Plan<E, DType> &plan, DType scale,
                                Shape<2> eshape, cudaStream_t stream) {
...
template <typename Saver, typename Reducer, int block_dim_bits, typename DType,
          typename DstPlan, typename Plan>
__global__ void MapReduceKeepDim1Kernel(DstPlan dst, Plan plan, DType scale,
                                        Shape<4> pshape) {
...
inline void MapReduceKeepDim1(expr::Plan<DstExp, DType> dst,
                              const expr::Plan<E, DType> &plan, DType scale,
                              Shape<4> pshape, cudaStream_t stream) {
  dim3 dimBlock(kBaseThreadNum);
  const int grid_dim_x = (pshape[1] > kMaxGridNum) ? kMaxGridNum : pshape[1];
  const int grid_dim_y = (pshape[1] > kMaxGridNum)
                             ? (pshape[1] + kMaxGridNum - 1) / kMaxGridNum
                             : 1;
...
__global__ void GetBatchedViewKernel(DType **dst, DType *src, int num,
                                     int stride) {
...
template <int x_bits, typename DType, typename DstPlan, typename SrcPlan1,
          typename SrcPlan2>
...
template <int x_bits, typename DType, typename DstPlan, typename SrcPlan1,
          typename SrcPlan2>
__global__ void SmoothSoftmaxGradKernel(DstPlan dst, SrcPlan1 src,
...
template <int x_bits, typename DType, typename DstPlan, typename SrcPlan1,
          typename SrcPlan2>
__global__ void SoftmaxGradKernel(DstPlan dst, SrcPlan1 src, SrcPlan2 label,
                                  index_t xmax, DType ignore_label) {
  const unsigned x_size = 1 << x_bits;
  const int y = blockIdx.x;
  const int k = static_cast<int>(label.Eval(0, y));
...
inline void Softmax(const Tensor<gpu, 2, DType> &dst,
  ...
  SoftmaxKernel<kBaseThreadBits, DType><<<dimGrid, dimBlock, 0, stream>>>(
      expr::MakePlan(dst), expr::MakePlan(src), dst.size(1));
...
inline void SoftmaxGrad(const Tensor<gpu, 2, DType> &dst,
  ...
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  SoftmaxGradKernel<kBaseThreadBits, DType><<<dimGrid, dimBlock, 0, stream>>>(
      expr::MakePlan(dst), expr::MakePlan(src), expr::MakePlan(label),
      dst.size(1));
...
inline void Softmax(const Tensor<gpu, 3, DType> &dst,
  ...
  CheckLaunchParam(dimGrid, dimBlock, "Softmax");
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  Softmax3DKernel<kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>>(dst, src);
...
inline void SoftmaxGrad(const Tensor<gpu, 3, DType> &dst,
  ...
  CheckLaunchParam(dimGrid, dimBlock, "SoftmaxGrad");
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  Softmax3DGradKernel<kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>>(dst, src, label);
...
inline void SoftmaxGrad(const Tensor<gpu, 3, DType> &dst,
  ...
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);
  Softmax3DGradKernel<kBaseThreadBits, DType>
      <<<dimGrid, dimBlock, 0, stream>>>(dst, src, label, ignore_label);
  MSHADOW_CUDA_POST_KERNEL_CHECK(Softmax3DGradKernel);
}

template <int x_bits, typename DType, typename DstPlan, typename SrcPlan1,
          typename SrcPlan2>
__global__ void AddTakeGradKernel(DstPlan dst, SrcPlan1 index, SrcPlan2 src,
...
__global__ void AddTakeGradLargeBatchKernel(DType *dst, const IdxType *sorted,
                                            const IdxType *index,
                                            const DType *src, int ymax,
                                            int xmax) {
  // Based on Torch's Version
  // https://github.com/torch/cunn/blob/master/lib/THCUNN/LookupTable.cu Each
  // warp is responsible for an input into the LookupTable. If the preceeding
  // input has the same as this input, then the warp exits immediately. The warp
  // also processes subsequent inputs with the same value.
  //
  // Input Warp
  // 1     <warp 1>
  ...
  if (idx < ymax && (idx == 0 || sorted[idx] != sorted[idx - 1])) {
    do {
...
inline void AddTakeGrad(Tensor<gpu, 2, DType> dst,
  ...
  AddTakeGradKernel<kUnitBits, DType><<<dimGrid, dimBlock, 0, stream>>>(
      expr::MakePlan(dst), expr::MakePlan(index), expr::MakePlan(src),
      src.size(0), src.size(1), K);
...
inline void AddTakeGradLargeBatch(Tensor<gpu, 2, DType> dst,
  ...
  const int grid_dim_y =
      (src.size(1) + block_dim_x * SZ - 1) / (block_dim_x * SZ);
  dim3 dimBlock(block_dim_x, block_dim_y);
  dim3 dimGrid(grid_dim_x, grid_dim_y);

  CHECK_EQ(dst.size(1), src.size(1)) << "AddTakeGradLargeBatch: shape mismatch";
  CHECK_EQ(index.size(0), src.size(0))
      << "AddTakeGradLargeBatch: shape mismatch";
  CheckLaunchParam(dimGrid, dimBlock, "AddTakeGradLargeBatch");
  cudaStream_t stream = Stream<gpu>::GetStream(dst.stream_);

  AddTakeGradLargeBatchKernel<kWarpBits, SZ, DType>
      <<<dimGrid, dimBlock, 0, stream>>>(
          dst.dptr_, sorted.dptr_, index.dptr_, src.dptr_,
          static_cast<int>(src.size(0)), static_cast<int>(src.size(1)));
  MSHADOW_CUDA_POST_KERNEL_CHECK(AddTakeGradLargeBatchKernel);
}

template <int warp_bits, typename DType, typename DstPlan, typename IndexPlan,
          typename SrcPlan>
__global__ void IndexFillKernel(DstPlan dst, const IndexPlan index,
                                const SrcPlan src, const int ymax,
                                const int xmax) {
  int bid = blockIdx.y * blockDim.x + blockIdx.x;
  int tid =
      bid * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
  if (tid < ymax * xmax) {
    int i = tid / xmax;
    int j = tid % xmax;
...
inline void IndexFill(Tensor<gpu, 2, DType> dst,
  ...
  IndexFillKernel<kMemUnitBits, DType><<<dimGrid, dimBlock, 0, stream>>>(
      expr::MakePlan(dst), expr::MakePlan(index), expr::MakePlan(src),
...
inline void SortByKey(Tensor<gpu, 1, KDType> keys,
                      Tensor<gpu, 1, VDType> values, bool is_ascend) {
  ...
  if (is_ascend) {
    thrust::stable_sort_by_key(thrust::cuda::par.on(stream), key_iter,
                               key_iter + keys.size(0), value_iter,
                               thrust::less<KDType>());  // NOLINT(*)
  } else {
    thrust::stable_sort_by_key(thrust::cuda::par.on(stream), key_iter,
                               key_iter + keys.size(0), value_iter,
                               thrust::greater<KDType>());  // NOLINT(*)
  }
...
inline void SortByKey(Tensor<gpu, 1, mshadow::half::half_t> keys,
                      Tensor<gpu, 1, DType> values, bool is_ascend) {
  LOG(FATAL) << "SortByKey for half_t is not implemented!";
}

template <typename DType>
inline void SortByKey(Tensor<gpu, 1, DType> keys,
