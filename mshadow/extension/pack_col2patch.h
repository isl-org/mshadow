...
struct PackColToPatchXExp
    : public MakeTensorExp<PackColToPatchXExp<SrcExp, DType, dstdim>, SrcExp,
                           dstdim, DType> {
  ...
  index_t pdilate_x_;
  /*! \brief constructor */
  PackColToPatchXExp(const SrcExp &src, Shape<dstdim> imshape, index_t psize_y,
    ...
    const index_t o_height =
        (imshape[dstdim - 2] - (pdilate_y * (psize_y - 1) + 1)) / pstride_y + 1;
    const index_t o_width =
        (imshape[dstdim - 1] - (pdilate_x * (psize_x - 1) + 1)) / pstride_x + 1;
...
/*!
 * \brief reverse operation of pack_col2patch, can be used to implement
 * deconvolution \return packed img expression \param mat source matrix \param
 * imshape shape of target img \param psize_y height of each patch \param
 * psize_x height of each patch \param pstride stride of each patch \tparam
 * SrcExp source expression \tparam DType the type of elements \tparam dstdim
 * destination dimension \tparam etype type of expression
 */
template <typename SrcExp, typename DType, int dstdim, int etype>
inline PackColToPatchXExp<SrcExp, DType, dstdim> pack_col2patch(
    const expr::Exp<SrcExp, DType, etype> &src, Shape<dstdim> imshape,
  ...
  return PackColToPatchXExp<SrcExp, DType, dstdim>(src.self(), imshape, psize_y,
                                                   psize_x, pstride, pstride,
                                                   pdilate, pdilate);
}
/*!
...
inline PackColToPatchXExp<SrcExp, DType, dstdim> pack_col2patch(
  ...
  CHECK(imshape[dstdim - 1] >= psize_x && imshape[dstdim - 2] >= psize_y)
      << "PackColToPatch:image shape smaller than patch size";
  return PackColToPatchXExp<SrcExp, DType, dstdim>(
      src.self(), imshape, psize_y, psize_x, pstride_y, pstride_x, pdilate_y,
      pdilate_x);
}
...
struct Plan<PackColToPatchXExp<SrcExp, DType, dstdim>, DType> {
 public:
  ...
  MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
    ...
    const index_t py_min = y < psize_y_dilate
                               ? y % pdilate_y_
                               : (y - psize_y_dilate + pstride_y_) / pstride_y_;
    const index_t px_min = x < psize_x_dilate
                               ? x % pdilate_x_
                               : (x - psize_x_dilate + pstride_x_) / pstride_x_;
    ...
    for (index_t py = py_min; py < py_max; py += pdilate_y_) {
      for (index_t px = px_min; px < px_max; px += pdilate_x_) {
        res += src_.Eval(
            ((c * psize_y_ + (y - py * pstride_y_) / pdilate_y_) * psize_x_ +
