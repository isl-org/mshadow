...
/*!
 * \brief Broadcasting the tensor in the given axis. If keepdim is off, insert
 * the broadcasting dim after axis. Otherwise broadcasting axis. \tparam SrcExp
 * source expression \tparam DType  data type \tparam dimsrc source dimension
...
broadcast_with_axis(const Exp<SrcExp, DType, etype> &src, const int axis,
                    const index_t size) {
...
inline BroadcastWithAxisExp<SrcExp, DType, ExpInfo<SrcExp>::kDim,
                            ExpInfo<SrcExp>::kDim>
broadcast_keepdim(const Exp<SrcExp, DType, etype> &src, const int axis,
                  const index_t size) {
  return BroadcastWithAxisExp<SrcExp, DType, ExpInfo<SrcExp>::kDim,
...
/*!
* \brief Broadcasting the tensor in multiple axes. The dimension of the source
tensor in the given axes must be 1.
* \tparam SrcExp source expression
* \tparam DType  data type
...
struct BroadcastWithMultiAxesExp
  ...
  BroadcastWithMultiAxesExp(const SrcExp &src, const TShape &axes,
                            const TShape &sizes)
      : src_(src) {
    Shape<dimsrc> src_shape = ShapeCheck<dimsrc, SrcExp>::Check(src_);
    CHECK(axes.ndim() == sizes.ndim())
        << "ndim of axes and sizes must be equal.";
    this->axesnum_ = axes.ndim();
    CHECK(this->axesnum_ <= dimsrc)
        << "Number of broadcasting axes must be smaller than"
           "the source ndim, number of axes="
        << this->axesnum_ << " dimsrc=" << dimsrc;
    for (index_t i = 0; i < this->axesnum_; i++) {
      CHECK(dimsrc > axes[i])
          << "broadcast axis (keepdim) out of bound, "
          << "all axes must be between 0 and" << dimsrc - 1 << ", given axes["
          << i << "] = " << axes[i] << ".";
      CHECK_EQ(src_shape[axes[i]], 1U)
          << "Size of the dimension of the broadcasting axis must be 1"
          << ", src_shape[" << axes[i] << "]=" << src_shape[axes[i]] << ".";
      if (i < this->axesnum_ - 1) {
        CHECK(axes[i] < axes[i + 1])
...
broadcast_multi_axes(const Exp<SrcExp, DType, etype> &src, const TShape &axes,
                     const TShape &sizes) {
  return BroadcastWithMultiAxesExp<SrcExp, DType, ExpInfo<SrcExp>::kDim>(
      src.self(), axes, sizes);
}
...
broadcast_to(const Exp<SrcExp, DType, etype> &src, const TShape &target_shape) {
  ...
  for (size_t i = 0; i < dimsrc; ++i) {
    if (src_shape[i] != target_shape[i]) {
      CHECK_EQ(src_shape[i], 1U)
          << "broadcasting axis must have size 1, received shape=" << src_shape
 ...
 public:
  explicit Plan(const BroadcastWithAxisExp<SrcExp, DType, dimsrc, dimdst> &e)
      : src_(MakePlan(e.src_)),
        dst_last_(e.dst_last_),
        trailing_(e.trailing_),
        size_(e.size_),
        last_(e.last_) {}
 ...
 public:
  ...
  MSHADOW_XINLINE DType Eval(index_t i, index_t j) const {
    ...
    for (index_t p = 0; p < dimsrc; ++p) {
      ...
      indx = (indx / trailings_[p] / sizes_[p]) * trailings_[p] +
             (indx % trailings_[p]);
    }
