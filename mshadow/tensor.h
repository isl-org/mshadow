 ...
 *  all the data structure Tensor<cpu,1>, Tensor<gpu,1> are like
 * handles(pointers), no memory allocation is happening during calculation
 *
...
#pragma unroll
   ...
   */
  MSHADOW_XINLINE index_t &operator[](index_t idx) { return shape_[idx]; }
  /*!
   * \brief get corresponding index
   * \param idx dimension index
   * \return the corresponding dimension size
 ...
 * \return the shape construction
 */
MSHADOW_XINLINE Shape<4> Shape4(index_t s0, index_t s1, index_t s2,
                                index_t s3) {
  Shape<4> s;
  s[0] = s0;
  s[1] = s1;
 ...
 * \return the shape construction
 */
MSHADOW_XINLINE Shape<5> Shape5(index_t s0, index_t s1, index_t s2, index_t s3,
                                index_t s4) {
  Shape<5> s;
  s[0] = s0;
  s[1] = s1;
 ...
 */
inline Shape<3> ConvertLayout(const Shape<3> &src, int src_layout,
                              int dst_layout) {
  Shape<3> dst;
  switch (src_layout) {
    case kNCW:
 ...
 * \return shape in target layout
 */
inline Shape<4> ConvertLayout(const Shape<4> &src, int src_layout,
                              int dst_layout) {
  Shape<4> dst;
  switch (src_layout) {
    case kNCHW:
 ...
 * \return shape in target layout
 */
inline Shape<5> ConvertLayout(const Shape<5> &src, int src_layout,
                              int dst_layout) {
  Shape<5> dst;
  switch (src_layout) {
    case kNCDHW:
...
struct Stream {
   ...
   */
  inline bool CheckIdle(void) { return true; }
...
struct Tensor : public TRValue<Tensor<Device, dimension, DType>, Device,
                               dimension, DType> {
 public:
  //--------------------------------
  // struct memembers
   ...
   *    this is used to deal with pitch allocation in gpu or sse(align x
   * dimension to 64bit) for efficiency
   */
  index_t stride_;
  /*!
...
#pragma unroll
   ...
   * \return the size
   */
  MSHADOW_XINLINE index_t size(index_t idx) const { return shape_[idx]; }
  /*!
   ...
   * together \return tensor after flatten
   */
   ...
   */
  MSHADOW_XINLINE Tensor<Device, dimension, DType> Slice(index_t begin,
                                                         index_t end) const {
  ...
  inline Tensor<Device, dimension, DType> &operator=(
      const Tensor<Device, dimension, DType> &exp) {
    dptr_ = exp.dptr_;
    shape_ = exp.shape_;
    stride_ = exp.stride_;
    stream_ = exp.stream_;
...
/*
 *  respecialized class Tensor1D, thei is due to different implementation in
 * operator[]
 */
...
struct Tensor<Device, 1, DType>
 ...
 public:
  ...
  MSHADOW_XINLINE Tensor<Device, 1, DType> Slice(index_t begin,
                                                 index_t end) const {
    Shape<1> s;
    s[0] = end - begin;
    return Tensor<Device, 1, DType>(dptr_ + begin, s, s[0], stream_);
  }
  MSHADOW_XINLINE bool CheckContiguous(void) const { return true; }
  MSHADOW_XINLINE index_t MSize(void) const { return shape_[0]; }
  MSHADOW_XINLINE index_t size(index_t i) const { return shape_[0]; }
  MSHADOW_XINLINE DType &operator[](index_t idx) { return dptr_[idx]; }
  MSHADOW_XINLINE const DType &operator[](index_t idx) const {
    return dptr_[idx];
  }
  /*!\brief implement the assignment of same type */
  inline Tensor<Device, 1, DType> &operator=(
      const Tensor<Device, 1, DType> &exp) {
    dptr_ = exp.dptr_;
    shape_ = exp.shape_;
    stride_ = exp.stride_;
    stream_ = exp.stream_;
...
inline Stream<Device> *NewStream(bool create_blas_handle,
                                 bool create_dnn_handle, int dev_id = -1);
 ...
 *            if true, will allocate space with stride_ that may not equals
 * shape[0] if false, will allocate continuous space \tparam dim specify the dim
 * of tensor \tparam DType type of element in tensor
 */
 ...
 * \param stream the stream, when specified, the copy can exhibit asynchronize
 * behavior \tparam dim specify the dim of tensor \tparam DType type of element
 * in tensor
 */
template <int dim, typename DType>
...
inline void Copy(Tensor<gpu, dim, DType> dst,
                 ...
                 Stream<gpu> *stream = NULL);
/*!
 * \brief CPU/GPU: normalize softmax: dst[i][j] = exp(energy[i][j]) /(sum_j
 * exp(energy[i][j])) \param dst destination \param energy input energy
 */
 ...
 * \brief CPU/GPU: Gradient accumulate of embedding matrix.
                   dst[index[i]] += src[i]
                   Called when the featuredim of src is much larger than the
 batchsize
 * \param dst destination
...
inline void AddTakeGradLargeBatch(Tensor<gpu, 2, DType> dst,
                                  ...
                                  const Tensor<gpu, 2, DType> &src);
/*!
 * \brief CPU/GPU: Fill the values of the destination matrix to specific rows in
 the source matrix. dst[index[i]] = src[i] Will use atomicAdd in the inner
 implementation and the result may not be deterministic.
 * \param dst destination
 ...
 * is performed!) \param keys the keys to sort \param values the values that
 * sorts w.r.t the key \param is_ascend whether to sort key in ascending order
 */
template <typename KDType, typename VDType>
inline void SortByKey(Tensor<cpu, 1, KDType> keys,
                      Tensor<cpu, 1, VDType> values, bool is_ascend = true);
/*!
 * \brief CPU/GPU: Sort key-value pairs stored in separate places. (Stable sort
 * is performed!) \param keys the keys to sort \param values the values that
 * sorts w.r.t the key \param is_ascend whether to sort key in ascending order
 */
template <typename KDType, typename VDType>
inline void SortByKey(Tensor<gpu, 1, KDType> keys,
                      Tensor<gpu, 1, VDType> values, bool is_ascend = true);
/*!
 * \brief CPU/GPU: Sort the keys within each segment. (Stable sort is
 performed!) Segments is defined as an ascending ordered vector like [0, 0, 0,
 1, 1, 2, 3, 3, 3,...] We sort separately the keys labeled by 0 and 1, 2, 3,
 etc. Currently only supports sorting in ascending order !!
 ...
 * \tparam dim dim of the tensor, during usage, there is no need to specify this
 * parameter \tparam DType the type of elements in the tensor \tparam E
 * specifies the expression type, not need to specify this parameter during
 * usage \tparam etype expression type \param dst destination \param exp
 * expression \sa namespace mshadow:sv, mshadow::op, mshadow::expr
 */
...
inline void MapExp(TRValue<R, gpu, dim, DType> *dst,
                   const expr::Exp<E, DType, etype> &exp);
/*!
 * \brief CPU/GPU: map a expression, do reduction to 1D Tensor in lowest
 * dimension (dimension 0) \tparam Saver specify storage method \tparam Reducer
 * specify a reducer method \tparam R specifies the storage type of the tensor
 * \tparam DType the type of elements in the tensor
 * \tparam E specifies the expression type, not need to specify this parameter
 * during usage \tparam etype expression type \param dst destination \param exp
 * expression \param scale scale the result before save \sa namespace
 * mshadow:sv, mshadow::op, mshadow::red, mshadow::expr
 */
...
inline void MapReduceKeepLowest(TRValue<R, gpu, 1, DType> *dst,
                                ...
                                DType scale = 1);
/*!
 * \brief CPU/GPU: map a expression, do reduction to 1D Tensor in third
 * dimension (dimension 2) \tparam Saver specify storage method \tparam Reducer
 * specify a reducer method \tparam R specifies the storage type of the tensor
 * \tparam DType the type of elements in the tensor
 * \tparam dimkeep the target dimension to be kept, should be larger than 0, for
 * 0, use MapReduceKeepLowest \tparam E specifies the expression type, not need
 * to specify this parameter during usage \tparam etype expression type \param
 * dst destination \param exp expression \param scale scale the result before
 * save \sa namespace mshadow:sv, mshadow::op, mshadow::red, mshadow::expr
 */
 ...
 * \param workspace Workspace for casting DType* to DType** (batched-view), must
 * have size >= 3 * batch_size
 */
